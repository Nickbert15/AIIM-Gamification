...
import AiOutputJudgePlayer from './AiOutputJudgePlayer'
...
export default function GamePreviewModal({ game, onClose }: Props) {
  ...
  if (game.game_json.format === 'ai_output_judge') {
    return (
      <>
        <style>{`
          .gpm-overlay {
        ...
        `}</style>
        <div className="gpm-overlay" onClick={(e) => { if (e.target === e.currentTarget) onClose() }}>
          <div className="gpm-card" style={{ maxWidth: 780 }}>
            <div className="gpm-header">
              <div>
                <h2 className="gpm-title">{game.title}</h2>
                <div className="gpm-subtitle">AI Output Judge · {game.difficulty}</div>
              </div>
              <button className="gpm-close" onClick={onClose}>×</button>
            </div>
            <AiOutputJudgePlayer game={game} onComplete={(s) => console.log('preview score:', s)} />
          </div>
        </div>
      </>
    )
